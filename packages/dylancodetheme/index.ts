...
export default function dylancodetheme(): StarlightPlugin {
  return {
    name: 'dylancodetheme',
    hooks: {
      'config:setup'({ config, logger, updateConfig }) {
         ...
         */
        logger.info('Hello from the dylancodetheme plugin!')

        /**
         * Update the provided Starlight user configuration by appending the theme CSS file to the `customCss` array.
         * To start customizing your theme, edit the `packages/dylancodetheme/styles.css` file.
         *
         ...
         */
        updateConfig({
          customCss: [...(config.customCss ?? []), 'dylancodetheme/styles'],
        })
      },
    },
  }
