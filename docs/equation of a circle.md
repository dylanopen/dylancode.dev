...
For any point that lies on the circle, the equation will be true and this
creates a circle if we would plot it using a graphing calculator, for example.
...
If we have a circle with a centre at the origin (0,0), then the equation would
be $(x-0)^2+(y-0)^2=r^2$, which simplifies to:
...
Knowing the equation above, we can use that to find the radius and/or the centre
coordinates of a circle, given just its equation.
...
We can also go the other way round - finding the equation of a circle if we know
