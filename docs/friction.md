...
It's basically when the surface of an object is rubbing against another surface,
causing a force which opposes the motion of the object.

A thing to keep in mind is that friction can only *oppose* an *existing* force
or motion. It can't *create* a force or motion on its own - so basically, it
can only slow down an object rather than actually make it move.
