...
A *bipartite graph* is a type of [graph](/graph) where the
[vertices](/graph-vertex) can be divided into two groups.

No two vertices within the same group are connected by an
[edge](/graph-edge). Instead, edges only connect vertices from one group to
