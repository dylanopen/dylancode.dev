...
- A *velocity-time graph* is a graph which shows:
  - the [velocity](/velocity) of an object on the y-axis
...
Or, we can just find the [derivative](/power-rule-for-differentiation) if we know an equation for the graph.
...
We can find this by finding the velocity at each of those times, and then using the normal [acceleration formula](/acceleration) to find the average
acceleration.
