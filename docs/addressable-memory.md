...
> Addressable memory is a way we can lay out the memory in a computer so that we can access it using addresses. Each piece of data or instruction is stored at a specific address in memory, and we can use that address to read or write data.
...
In this example, we have a total of 5 bytes stored in memory. Each *byte* has a unique address (0x00, 0x01, etc.) that we can use to access it. So if we want to read the data at address 0x02, we would get the value 'L'. If we want to write a new value to address 0x03, we can do that as well.
