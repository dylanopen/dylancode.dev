...
We calculate it by adding up all the numbers in the set, and then dividing that
total by the number of values in the set:

## Calculating mean

1. Add up all the numbers in the set to get the total.
1. Count how many numbers there are in the set to get the number of values.
1. Divide the total by the number of values to get the mean.

## Notation for mean

We can represent the mean of a set of numbers $x$ using the symbol $\overline{x}$
(x with a bar on top):
