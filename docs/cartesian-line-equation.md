...
This equation is true, because any $(x,y)$ point along the line will satisfy
the equation.

For example, take the line equation $y=4x+3$, which, drawing the graph, we
can see has a point at, for example, $(2, 11)$:
...
> - A line equation simply represents a 'condition' where, when a point is
>   substituted into the line equation, the point is only on the line if the
>   equation is satisfied (true).
...
We don't usually work with 3D lines in their Cartesian format (we usually
[convert them to vector form](/converting-cartesian-to-vector-line)) - but
we can do.

The same rule applies as with 2D lines - the equation of a line simply
defines the conditions that a point must satisfy to be on the line.
