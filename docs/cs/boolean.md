...
For example, we could use a boolean to represent whether a light switch is on or
off.
...
Remember that the only two possible values for a boolean are `true` and `false`:
we couldn't set `isLightOn` to `1` or `"yes"`, for example.

## Uses of booleans

Booleans are mainly used for *conditional statements* - for example, deciding
